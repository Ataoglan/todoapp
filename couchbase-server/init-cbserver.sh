...
if ! [ -f "$FILE" ]; then
  ...
  cbq -e http://localhost:8093 -u $COUCHBASE_ADMINISTRATOR_USERNAME -p $COUCHBASE_ADMINISTRATOR_PASSWORD \
      --script='INSERT INTO `todoapp` (KEY, VALUE) VALUES ("user_id_counter", {"counter": 0})'

  # Couchbase sunucusunu çalıştır
